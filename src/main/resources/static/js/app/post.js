const main = {
    ...
    createParent: function () {
        ...
        if(tmp === '') {
            alert('댓글을 입력해 주세요.');
            $('#opinionContent').summernote({focus: true});
            return false;
        }
        ...
        $.ajax({
            ...
            let replyItem = '<div id="parentList'+result.id+'">\n' +
                ...
                '                  <button class="border-0 px-0" style="background-color: transparent;" onclick="main.likeOpinion('+result.id+')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-up text-primary" viewBox="0 0 16 16">\n' +
                ...
                '                  <button class="border-0 px-0" style="background-color: transparent;" onclick="main.disLikeOpinion('+result.id+')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-down text-danger" viewBox="0 0 16 16">' +
                ...
                '                       <button class="border-0 float-right ml-1" data-toggle="dropdown" style="background-color: transparent;">\n' +
                ...
                '              </div>\n' +
                '            </div>\n' +
                '            <div class="pt-3 contentField">'+result.content+'</div>'+
                '            <div class="row">\n' +
                '              <div class="col text-right">\n' +
            ...
            $('#opinionContent').summernote('reset')
        }).fail(function (error){
            alert('등록 실패');
        });
    },
    ...
    createTextField: function (parentId, userPicture) {
        ...
        $('#childOpinionContent').summernote({
            height : 80,
            minHeight : null,
            maxHeight : null,
            focus : true,
    ...
    createChild: function () {
        ...
        if(tmp === '') {
            alert('댓글을 입력해 주세요.');
            $('#childOpinionContent').summernote({focus: true});
            return false;
        }
        ...
        $.ajax({
            ...
            $('#childOpinionContent').summernote('reset');
        }).fail(function (error){
            alert('등록 실패');
        });
    },
    ...
    createUpdateField : function (id, isParent) {
        ...
        if($('#'+id).find('textarea').length === 2) {
            return;
        } // 수정창 중복생성 방지

        const content = $('#'+id).find('div.contentField').html();
        ...
        const replyItem = '<div>' +
            ...
            '                </div>\n' +
            '              </div>';
        $('#'+id).find('div.contentField').hide(); // 텍스트 감추기
        if(isParent) {
            $('#'+id).children().last().hide(); // 대댓글 버튼 감추기
        }

        $('#'+id).append(replyItem); // 수정창 추가
        $('#'+id).find('textarea').summernote({
            height : 80,
            minHeight : null,
            maxHeight : null,
            focus : true,
    ...
    updateOpinion : function (id) {
        ...
        if($('#'+id).find('div.contentField').html() === data.content) {
        ...
        $.ajax({
            ...
            $('#'+id).find('div.contentField').html(data.content);
